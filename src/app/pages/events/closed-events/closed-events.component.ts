...
@Component({
  selector: "app-closed-events",
  templateUrl: "./closed-events.component.html",
  styleUrls: ["./closed-events.component.css"],
  standalone: true,
  imports: [
    CommonModule,
...
export class ClosedEventsComponent implements OnInit, OnDestroy {
  ...
  onHeaderFilterChange(type: string, value: any) {
    ...
    if (type === 'site') {
      ...
      if (value) {
        ...
        // Revert to Global Action Tag Categories for "All Sites"
  ...
  private resolveMediaUrls(item: any): string[] {
    ...
    // ✅ Map everything through buildImageUrl
    return rawUrls
      .filter(x => !!x)
  ...
  /** NEW: From child – "+ ADD" button or Pencil */
  ...
  submitAddComment(): void {
    ...
    if (!eventId) {
      console.error("No eventId on selectedItem", this.selectedItem);
    ...
    const performAddComment = () => {
      const remarks = this.addCommentForm.tag
        ? `Tag: ${this.addCommentForm.tag} | Added via escalation drawer`
        : "Added via escalation drawer";
      ...
      this.eventsService.addComment(commentPayload).subscribe({
        next: (res) => {
          ...
          this.showSuccess("Success", "Record updated and comment added.");
          this.isAddCommentPopupVisible = false;
          this.isTablePopupVisible = true;
        ...
        error: (err) => {
          this.isSubmittingComment = false;
          const msg = err?.error?.message || err?.error?.msg || "Failed to save comment.";
          this.showError("Add Comment Failed", msg);
        },
      });
    };

    // 1️⃣ UPDATE ESCALATION RECORD (If Closed tab and ID exists)
    if (this.selectedFilter === "CLOSED" && this.selectedEscalationRow?.id) {
      const updatePayload = {
      ...
      this.eventsService.putEventsMoreInfo(updatePayload).subscribe({
        ...
        error: (err) => {
          this.isSubmittingComment = false;
          console.error("Failed to update escalation record from parent drawer", err);
          this.showError("Update Failed", "Escalation update failed. Comment not added.");
        }
      });
    } else {
      // ✅ No update needed, just add comment
      performAddComment();
    }
  }
  ...
  mailselectitem: any;
  async openMailTooltip(event: MouseEvent, params: any) {
    this.mailselectitem = params.data;

    if (this.mailselectitem?.mailColour === 1) {
      const confirmed = await this.notification.confirm("Mail already sent Do you want to Send Again ?", {
        acceptLabel: "SEND AGAIN",
        rejectLabel: "CANCLE",
        header: "Confirmation"
      });
      if (!confirmed) return;
    ...
    this.closedColumnDefs = [
      ...
      },
      {
        ...
        cellRenderer: (params: any) => {
          ...
          return `
    <span class="material-symbols-outlined mail" title="${tooltip}" ${disableClick}   
          style="color:${color} !important; margin-right:8px; cursor:pointer; opacity:1;">
      mail
    </span>
